...
async def start_remote_pair_task(device_name: str | None) -> None:
    if start_tunnel is None:
        raise NotImplementedError("failed to start the tunnel on your platform")
...
@cli.command("pair")
def cli_pair(name: Annotated[str | None, typer.Option(help="Device name for a specific device to look for")]) -> None:
