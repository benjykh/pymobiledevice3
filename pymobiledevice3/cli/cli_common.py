...
def print_json(buf, colored: bool | None = None, default=default_json_encoder) -> str:
    if colored is None:
...
async def get_mobdev2_devices(udid: str | None = None) -> list[TcpLockdownClient]:
    return [lockdown async for _, lockdown in get_mobdev2_lockdowns(udid=udid)]


async def _tunneld(udid: str | None = None) -> RemoteServiceDiscoveryService | None:
    if udid is None:
        return
...
def make_rsd_dependency(*, allow_none: bool) -> Callable[..., RemoteServiceDiscoveryService | None]:
    def rsd_dependency(
        rsd: Annotated[
            tuple[str, int] | None,
            typer.Option(
...
RSDServiceProviderDep: TypeAlias = Annotated[
    RemoteServiceDiscoveryService,
    Depends(make_rsd_dependency(allow_none=False)),
]


ServiceProviderDep: TypeAlias = Annotated[
    LockdownServiceProvider,
    Depends(any_service_provider_dependency),
]


NoAutoPairServiceProviderDep: TypeAlias = Annotated[
    LockdownServiceProvider,
