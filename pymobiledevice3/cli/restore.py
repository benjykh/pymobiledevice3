...
from typing import IO, Annotated, TypeAlias
...
IPSWCtxDep: TypeAlias = Annotated[
    contextlib.AbstractContextManager[ZipFile],
...
async def restore_update_task(device: Device, ipsw: ZipFile, tss: dict | None, erase: bool, ignore_fdr: bool) -> None:
...
async def restore_tss_task(
    device: Device, ipsw_ctx: contextlib.AbstractContextManager[ZipFile], out: IO | None
) -> None:
...
@cli.command("tss")
def restore_tss(device: DeviceDep, ipsw_ctx: IPSWCtxDep, out: Path | None) -> None:
    """query SHSH blobs"""
